        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-8">
            <div className="inline-flex items-center gap-2 bg-gradient-to-r from-purple-600/20 to-pink-600/20 rounded-full px-6 py-2 mb-6 border border-purple-500/30">
              ...
              <span className="text-purple-300 text-sm font-medium">Decentralized Investment Platform</span>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
            {fundTypes.map((fund) => {
              ...
              return (
                <div
                  key={fund.id}
                  className={`group relative bg-gradient-to-br ${fund.bgGradient} backdrop-blur-xl rounded-2xl border ${fund.borderColor} ${fund.hoverBorder} transition-all duration-200 hover:scale-[1.02] hover:shadow-2xl ${fund.glowColor} overflow-hidden`}
                  onMouseEnter={() => setHoveredCard(fund.id)}
                  onMouseLeave={() => setHoveredCard(null)}
                >
                  {/* Animated border gradient */}
                  <div className={`absolute inset-0 bg-gradient-to-r ${fund.gradient} opacity-0 group-hover:opacity-20 transition-opacity duration-500 rounded-2xl`}></div>
                  
                  {/* Content */}
                  <div className="relative p-8 h-full flex flex-col">
                    {/* Header */}
                    <div className="flex items-center justify-between mb-6">
                      <div className={`p-3 rounded-xl bg-gradient-to-r ${fund.gradient} shadow-lg`}>
                        <IconComponent className="w-8 h-8 text-white" />
                      </div>
                      <div className="text-right">
                        <div className={`text-sm font-medium ${fund.color} mb-1`}>
                          {fund.type}
                        </div>
                    ...
                    <button
                      ...
                      className={`w-full py-4 rounded-xl bg-gradient-to-r ${fund.gradient} text-white font-semibold transition-all duration-300 hover:shadow-lg hover:shadow-purple-500/25 flex items-center justify-center gap-2 cursor-pointer`}>
                  ...
                  <div className={`absolute inset-0 bg-gradient-to-br ${fund.gradient} opacity-0 group-hover:opacity-5 transition-opacity duration-500 rounded-2xl pointer-events-none`}></div>
                </div>
              );
            })}
          </div>
          ...
          <div className="text-center">
            <div className="inline-flex items-center gap-4 bg-gradient-to-r from-gray-800/50 to-gray-700/50 backdrop-blur-xl rounded-2xl px-8 py-6 border border-gray-600/30">
              <div className="flex items-center gap-2">
                <Zap className="w-5 h-5 text-yellow-400" />
                <span className="text-gray-300">Lightning Fast Setup</span>
              </div>
              <div className="w-px h-6 bg-gray-600"></div>
              <div className="flex items-center gap-2">
                <Vote className="w-5 h-5 text-blue-400" />
                <span className="text-gray-300">Governance Ready</span>
              </div>
              <div className="w-px h-6 bg-gray-600"></div>
              <div className="flex items-center gap-2">
                <Coins className="w-5 h-5 text-green-400" />
                <span className="text-gray-300">Multi-Asset Support</span>
              </div>
            </div>
          </div>
        </div>
