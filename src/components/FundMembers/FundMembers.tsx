...
return (
<div className="bg-gradient-to-b from-[#1f2937] to-[#0f172a] p-6 h-[28rem] w-[25%] relative overflow-hidden rounded-2xl shadow-[0_0_20px_#6366f155] border border-white/10 transition-all duration-500">
  {/* Loading Skeleton */}
  {loading ? (
    <div className="animate-pulse space-y-4 overflow-y-auto pr-2 h-[calc(100%-9.5rem)]">
    ...
    <>
      {/* Title */}
      <h2 className="text-xl font-semibold mb-4 text-white tracking-tight flex items-center gap-2">
        👥 Members
      </h2>

      {/* Scrollable List */}
      <div className="overflow-y-auto pr-2 h-[calc(100%-9.5rem)] fancy-scrollbar space-y-4 text-sm">
        <ul className="space-y-4">
  ...
  {/* Bottom Buttons */}
  <div className="absolute bottom-0 left-0 w-full bg-[#111827] pt-3 pb-4 px-4 space-y-3 border-t border-gray-700 z-10">
    {['Deposit', 'Invite', 'Withdraw'].map((label, idx) => (
      <button
      onClick={() => {
        if (label === 'Deposit') {
          openDepositModal();
        ...
        key={idx}
        className="w-full py-2 rounded-lg text-sm font-semibold text-white bg-gradient-to-r from-purple-600 to-indigo-500 hover:from-purple-700 hover:to-indigo-600 transition duration-300 shadow-md hover:shadow-lg"
      >
        {label}
      </button>
    ))}
